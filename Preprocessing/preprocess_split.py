import os
import re
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.utils import resample

# ----------------------------
# CONFIGURE YOUR PATHS
# ----------------------------
adversarial_sub = "/home/deepaksg/Downloads/Banking-LLM-Security-main/Dataset(Not Preprocessed)/Adversarial"
benign_sub = "/home/deepaksg/Downloads/Banking-LLM-Security-main/Dataset(Not Preprocessed)/Benign"

# ----------------------------
# IMPROVED CLEAN TEXT FUNCTION
# ----------------------------
def clean_text(text):
    if pd.isna(text):
        return ""
    if not isinstance(text, str):
        text = str(text)
    # Check if it's likely a numeric ID or code (not actual text)
    if text.strip().isdigit() or len(text.strip()) <= 3:
        return ""
    text = text.lower()
    text = re.sub(r"http\S+|www\S+|https\S+", "", text)
    text = re.sub(r"\d+", "", text)
    text = re.sub(r"[^a-zA-Z\s]", "", text)
    text = re.sub(r"\s+", " ", text).strip()
    return text

# ----------------------------
# HELPER: GET TEXT COLUMN
# ----------------------------
def get_text_column(df):
    candidates = ["text", "utterance", "prompt", "query", "content", "instruction", "input", "message"]
    for col in df.columns:
        if col.lower() in candidates:
            return col
    # If no candidate found, try to find any string column
    for col in df.columns:
        if df[col].dtype == 'object':
            return col
    return df.columns[0]

# ----------------------------
# LOAD BENIGN DATA
# ----------------------------
print("Loading benign data...")
benign_df = pd.read_csv(os.path.join(benign_sub, "banking77_combined.csv"))
benign_text_col = get_text_column(benign_df)
benign_df = benign_df.rename(columns={benign_text_col: "text"})
benign_df["text"] = benign_df["text"].apply(clean_text)
benign_df = benign_df[benign_df["text"].str.strip() != ""]  # Remove empty
benign_df["label"] = 0
print(f"Benign data loaded: {benign_df.shape[0]} samples")

# ----------------------------
# LOAD ADVERSARIAL DATA
# ----------------------------
print("\nLoading adversarial data...")
adv_dfs = []
for f in os.listdir(adversarial_sub):
    if f.endswith(".csv"):
        try:
            temp = pd.read_csv(os.path.join(adversarial_sub, f))
            text_col = get_text_column(temp)
            temp = temp.rename(columns={text_col: "text"})
            temp["text"] = temp["text"].astype(str).apply(clean_text)
            temp = temp[temp["text"].str.strip() != ""]
            temp["label"] = 1
            adv_dfs.append(temp[["text", "label"]])
            print(f"Loaded {f}: {temp.shape[0]} samples")
        except Exception as e:
            print(f"Error loading {f}: {e}")

adversarial_df = pd.concat(adv_dfs, ignore_index=True)
print(f"Total adversarial samples: {adversarial_df.shape[0]}")

# ----------------------------
# OVERSAMPLE BENIGN DATA TO MATCH ADVERSARIAL
# ----------------------------
print(f"\nBefore oversampling:")
print(f"Benign samples: {benign_df.shape[0]}")
print(f"Adversarial samples: {adversarial_df.shape[0]}")

# Oversample benign data to match adversarial count
benign_oversampled = resample(benign_df,
                             replace=True,  # Sample with replacement
                             n_samples=adversarial_df.shape[0],  # Match adversarial count
                             random_state=42)

print(f"\nAfter oversampling:")
print(f"Benign samples: {benign_oversampled.shape[0]}")
print(f"Adversarial samples: {adversarial_df.shape[0]}")

# ----------------------------
# MERGE BOTH DATASETS
# ----------------------------
combined_df = pd.concat([benign_oversampled, adversarial_df], ignore_index=True)

# Shuffle the dataset
combined_df = combined_df.sample(frac=1, random_state=42).reset_index(drop=True)

print(f"\n=== FINAL DATASET STATISTICS ===")
print(f"Total samples: {combined_df.shape[0]}")
print(f"Benign samples (label 0): {(combined_df['label'] == 0).sum()}")
print(f"Adversarial samples (label 1): {(combined_df['label'] == 1).sum()}")

# ----------------------------
# TRAIN/TEST SPLIT
# ----------------------------
train_df, test_df = train_test_split(combined_df, test_size=0.2, stratify=combined_df["label"], random_state=42)

print(f"\n=== TRAIN/TEST SPLIT ===")
print(f"Training set: {train_df.shape[0]} samples")
print(f"Test set: {test_df.shape[0]} samples")
print(f"Training benign: {(train_df['label'] == 0).sum()}")
print(f"Training adversarial: {(train_df['label'] == 1).sum()}")
print(f"Test benign: {(test_df['label'] == 0).sum()}")
print(f"Test adversarial: {(test_df['label'] == 1).sum()}")

# ----------------------------
# TF-IDF VECTORIZATION
# ----------------------------
vectorizer = TfidfVectorizer(max_features=5000, ngram_range=(1,2))
X_train = vectorizer.fit_transform(train_df["text"])
X_test = vectorizer.transform(test_df["text"])

y_train = train_df["label"].values
y_test = test_df["label"].values

print(f"\nTF-IDF train shape: {X_train.shape}")
print(f"TF-IDF test shape: {X_test.shape}")

# ----------------------------
# SAVE DATA
# ----------------------------
train_df.to_csv("train.csv", index=False)
test_df.to_csv("test.csv", index=False)
print("✅ Saved train.csv and test.csv")

import joblib
joblib.dump(X_train, "X_train.pkl")
joblib.dump(X_test, "X_test.pkl")
joblib.dump(y_train, "y_train.pkl")
joblib.dump(y_test, "y_test.pkl")
joblib.dump(vectorizer, "vectorizer.pkl")
print("✅ Saved TF-IDF features and vectorizer")